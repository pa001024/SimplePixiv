...
// ==/UserScript==
(function($) {
    if (!window.localStorage) alert("您的浏览器版本过旧,不支持本地数据(localStorage),将无法使用一部分功能!");
    var Store = function(n) {
        if (!window.localStorage[n]) window.localStorage[n] = "";
        var hs = window.localStorage[n].split(","), t;
        this.load = function() { hs = window.localStorage[n].split(",") };
        this.get = function(k) { return (t = hs.indexof(k)) > 0 ? hs[t] : null; };
        this.set = function(k, v) { if (!this.get(k)) hs.push(v); return this }
        this.save = function(k, v) { window.localStorage[n] = hs.join(","); return this }
    };

    var igonestore = new Store("bkm");
    var namestore = new Store("ikm");
    this.SimplePixiv = { version: "1.22" };

    function gname(f, o) {
        if (!f) f = "?id? - ?iname? ?uname??iyear?";
        for (var i in o) f.replace("?" + i + "?", o[i]);
        return f;
    }

    function getpagec(c, d) {
        if (!d) d = 20;
        return c % d == 0 ? c / d : Math.floor(c / d) + 1
    }
        ...
        if (1) {
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(f) {
                    ...
                    //f.responseHeaders.match("Content-Length: (.+)")[1]/1);
                    //for(var i in d)if((t=d.charCodeAt(i))>255){n[j++]=t>>8;n[j++]=t&255}else n[j++]=t;
    ...
    $.D = !1;
    $(".ad,.user-ad-container,.ads_area_no_margin,#google_ads_frame1").remove(); //去广告
    var //设置
        fmt = "?id? - ?iname?" //保存格式 可用:作品ID:?id? 作品标题:?iname? 作者ID:?uid? 作者名:?uname? 下载时间:?dyear? ?dmouth? ?dday? 上传时间:?itime? ?imouth? ?iday?
        //环境
        ,
        ...
        css4 = 'box-shadow:0 0 100px red inset,0 0 20px red;border:0',
    ...
    if (ph == "/member_illust.php" || (ph == "/bookmark.php" && /[&?]id=\d+/.test(url) || url.indexOf("type=user") > 0)) {
        var ajm = url.indexOf("type=user") > 0 ? getpagec($(".info .count").text(), 48) : getpagec(/\d+/.exec($(".layout-inline-block .count-badge").text())),
            s = ".linkStyleWorks > ul,.members > ul";
        $(".pages:last").html(loadimg);
        if (ph == "/bookmark.php" && url.indexOf("type=user") > 0 && url.indexOf("id=") > 0)
            window.onfocus = (function() {
                var f = 1;
                function a() {
                    if (!window.localStorage) return;
                    if (!window.localStorage["bkm"]) window.localStorage["bkm"] = "";
                    var a = window.localStorage["bkm"].split(",");
                    for (var i in a) $(document.querySelector("a[href='member.php?id=" + a[i] + "']")).parent().parent().hide(1200, function() {
                        $(this).remove()
                    });
                    $(document.querySelectorAll(".users a")).attr("target", "_blank");
                    //console.log("[onfocus]:已从画师列表中移去忽略部分");
                }
                return function(e) {
                    if ((f + 1e3) < (f = e.timeStamp)) a()
                };
            })();
        if (url.indexOf("type=user") < 0 || url.indexOf("id=") < 0) {
            if (ph == "/member_illust.php" && url.indexOf("mode") < 0) {
                ...
                var a = window.localStorage["bkm"].split(","),
                    c = location.search.match(/[&?]id=(\d+)/)[1],
                    hs = a.indexOf(c);
                $(".pages:first").html("")
                    .append($("<a style='" + css1 + "' href='javascript:;'>收图模式 OFF</a>").attr("onclick", "$(this).html('收图模式 '+(($.D=!$.D)?'ON':'OFF'));if($.D)$('#PiP').show();else $('#PiP').hide()"))
                    .append($("<a style='" + css1 + "'></a>")
                        .html(hs < 0 ? "×忽略" : "已忽略")
                        .attr("href", 'javascript:localStorage["bkm"]+=","+location.search.match(/[&?]id=(\\d+)/)[1];window.opener=null;window.open("","_self");window.close();void 0'));
                ...
                parent.D = function(a) {
                    ...
                    dlimg(b.attr("src").replace(/(\d+)_s(\.(?:png|jpg))/, "$1$2"), b.attr("src").split("/").pop().replace("_s.", " - " + b.parent().text() + '.').replace(/\?.+$/, ""), a);
                    return !1
                }
